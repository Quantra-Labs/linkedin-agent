...
export class CadenceService {
	...
	async processDueAssignments(): Promise<number> {
		const now = new Date();
	...
	async executeNextStep(assignmentId: string): Promise<void> {
		...
		if (!assignment || !assignment.sequence || !assignment.lead) return;
		...
		if (step.action === 'SEND_CONNECTION') {
			...
			await prisma.outboundMessage.create({
				data: {
					...
					content: result.ok ? 'Connection requested' : ('reason' in result ? (result as any).reason : 'Connection scheduled'),
					sentAt: result.ok ? new Date() : null,
					error: result.ok ? null : ('reason' in result ? (result as any).reason : null),
				},
			});

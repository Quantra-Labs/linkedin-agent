...
export type MessageContext = {
	...
	template?: string; // handlebars-style: {{firstName}}
	systemPrompt?: string;
};

function applyTemplate(template: string, ctx: MessageContext): string {
	return template.replace(/{{\s*(\w+)\s*}}/g, (_m, key) => {
		const value = (ctx.lead as any)[key];
		return value ?? '';
	});
}
...
export class AiService {
	...
	async generateMessage(ctx: MessageContext): Promise<string> {
		if (ctx.template) {
